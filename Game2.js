...
////////////////// Character chosen /////////////////
var character = localStorage.getItem("character1");
var level5 = localStorage.getItem("level2");
var monkeytrails = document.getElementsByClassName("monks");
var ostrishtrails = document.getElementsByClassName("ostrish");
var tries = 0;
var livetrails=0;
...
if (character === '0') {
    if (level5 === '1') {
        ...
        for (let i = 0; i < monkeytrails.length; i++) {
            monkeytrails[i].style.display = "inline";
            livetrails=6;
        }
    }
    else if (level5 === '2') {
        for (let i = 0; i < (monkeytrails.length - 2); i++) {
            monkeytrails[i].style.display = "inline";
            livetrails=4;
        }
    }
    else if (level5 === '3') {
        for (let i = 0; i < (monkeytrails.length - 4); i++) {
            monkeytrails[i].style.display = "inline";
            livetrails=2;
        }
    }
    ...
    if (level5 === '1') {
        for (let i = 0; i < ostrishtrails.length; i++) {
            ostrishtrails[i].style.display = "inline";
            livetrails=6;
        }
    }
    else if (level5 === '2') {
        for (let i = 0; i < (ostrishtrails.length - 2); i++) {
            ostrishtrails[i].style.display = "inline";
            livetrails=4;
        }
    }
    else if (level5 === '3') {
        for (let i = 0; i < (ostrishtrails.length - 4); i++) {
            ostrishtrails[i].style.display = "inline";
            livetrails=2;
        }
    }
...
function word_wrong() {
    ...
    var textbox5=localStorage.getItem("textbox4")
    var endmessage = document.getElementById("endmessage");
    endmessage.innerHTML = `${textbox5} YOU LOST! <br> Try Again`;
